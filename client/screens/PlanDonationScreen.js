...
export default function PlanDonationScreen() {
  ...
  const handleSubmit = async () => {
    ...
    // 1. Fetch user's last completed donation
    const q = query(
      collection(db, "donationRequests"),
    ...
    // 2. Calculate next possible date
  ...
  // Fetch next possible donation date when date picker opens
  useEffect(() => {
              ...
              <ScrollView style={styles.modalScroll}>
                {bloodRequests.length === 0 && (
                  <Text style={{ textAlign: "center", marginTop: 20 }}>
                    Немає запитів для цього центру
                  </Text>
                )}
